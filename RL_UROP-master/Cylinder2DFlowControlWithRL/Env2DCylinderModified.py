...
class Env2DCylinderModified(gym.Env):
    ...
    def __init__(self, path_root, geometry_params, flow_params, solver_params, output_params,
        ...
        transition_matrices = np.eye(64)  # You might need to modify this based on your system's dynamics
        observation_matrices = np.eye(64)  # Assuming you observe all state variables directly
    ...
    def step(self, actions=None):
        ...
        for crrt_control_nbr in range(self.number_steps_execution):
            ...
            self.probes_values = self.ann_probes.sample(self.u_, self.p_).flatten()
            self.probes_values = self.probes_values + np.random.normal(noise_mean, noise_stddev, self.probes_values.shape)
            #self.probes_values_velocity = self.ann_probes_velocity.sample(self.u_, self.p_).flatten()
            self.drag = self.drag_probe.sample(self.u_, self.p_)
            self.lift = self.lift_probe.sample(self.u_, self.p_)
            self.recirc_area = self.area_probe.sample(self.u_, self.p_)
            if crrt_control_nbr%5 ==0:

                     if measurements.size == 0:
                            measurements = self.probes_values.reshape(1, -1)  # Reshape to 2D (1 row, many columns)
                     else:
                            measurements = np.vstack((measurements, self.probes_values))
                    
                     count = count+1

            if crrt_control_nbr == int(self.number_steps_execution/2):

                     interreward = self.compute_reward()   
            #if crrt_control_nbr == int(self.number_steps_execution/2):
